...
double variationalWaveFunction_second(double v)
{
  double mu=0.8;
  double sigma=0.6;
  double FTrial=exp(-pow(v-mu,2.)/(2*pow(sigma,2.)))+exp(-pow(v+mu,2.)/(2*pow(sigma,2.)));
	//return 0;
  return (-1/pow(sigma,2.)*FTrial + pow(v-mu,2.)/pow(sigma,4.)*exp(-.5*pow((v-mu)/sigma,2.)) + pow(v+mu,2.)/pow(sigma,4.)*exp(-.5*pow((v+mu)/sigma,2.)));
