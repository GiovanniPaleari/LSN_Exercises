...
void Measure()
{
  ofstream Inst_U, Inst_P;

  //int bin;
  double v = 0.0, w = 0.0;
  double vij, wij;
  double dx, dy, dz, dr, dr_norm;
  //double left, right;
...
// distance i-j in pbc
      ...
      dr_norm = 2*dr/box*100;   //rescaling of distances from [0,box/2] to [0,100]

//update of the histogram of g(r)

      walker[2+(int)floor(dr_norm)] += 2.;

//modo normale ma non furbissimo di riempire l'istogramma
      /*for (int i=0; i<nbins; i++){
        left = i*bin_size;
        right = (i+1)*bin_size;
        if(dr>left && dr<right) walker[i+2] += 2.0;
      }*/
...
/*
  ...
  if(rho==1.1) phase = "_solid";
  else if (rho==0.8) phase = "_liquid";
  else if (rho==0.05) phase = "_gas";

  Inst_U.open("inst_energy" + phase + ext, ios::app);
  Inst_P.open("inst_pressure" + phase + ext, ios::app);

  Inst_U << mcstep << "\t" << walker[iv]/(double)npart + vtail << endl;
  Inst_P << mcstep << "\t" << rho * temp + (walker[iw] + (double)npart * ptail) / vol << endl;
...
//g(r)
  for (int k=0; k<nbins; k++){
    delta_v = 4./3.*pi*(pow(bin_size*(k+1),3.)-pow(bin_size*k,3.));
    gdir = blk_av[k+2]/blk_norm/(rho*npart*delta_v);
    glob_av[k+2] += gdir;
    glob_av2[k+2] += gdir*gdir;
    err_gdir=Error(glob_av[k+2],glob_av2[k+2],iblk);
    Gofr << setw(wd) << iblk << setw(wd) << (k+.5)*bin_size << setw(wd) << gdir <<  endl;
    if(iblk==nblk){
      Gave << setw(wd) << (k+.5)*bin_size << setw(wd) << glob_av[k+2]/(double)iblk << setw(wd) << err_gdir << endl;
    }
  }
