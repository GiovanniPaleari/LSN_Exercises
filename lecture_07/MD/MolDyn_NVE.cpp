...
void Measure(){ //Properties measurement
  ...
  //int bin;
  double v=0.0, t=0.0, pr=0.0, vij, prij;
  ...
  for (int i=0; i<npart-1; ++i)
  {
    for (int j=i+1; j<npart; ++j)
    {
      ...
      dr_norm = 2*dr/box*100;
      walker[igofr+(int)floor(dr_norm)] += 2.;
...
{
  double gdir;
  ofstream Gofr, Gave, Epot, Ekin, Etot, Temp, Pr;
  const int wd=12;
  double delta_v;
  ...
  stima_pr = rho*stima_temp + blk_av[ip]/blk_norm*rho/(double)(3*npart); //pressure
  glob_av[ip] += stima_pr;
  ...
  //Pressure
    Pr << setw(wd) << iblk <<  setw(wd) << stima_pr << setw(wd) << glob_av[ip]/(double)iblk << setw(wd) << err_pr << endl;
  //g(r)
    for (int k=0; k<nbins; k++){
      delta_v = 4./3.*pi*(pow(bin_size*(k+1),3.)-pow(bin_size*k,3.));
      gdir = blk_av[k+igofr]/blk_norm/(rho*npart*delta_v);
      ...
      if(iblk==nblocks){
        Gave << setw(wd) << (k+.5)*bin_size << setw(wd) << glob_av[k+igofr]/(double)iblk << setw(wd) << err_gdir << endl;
      }
    }
